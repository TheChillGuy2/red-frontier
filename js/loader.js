...
function getDrinkId() { const params = new URLSearchParams(window.location.search); return params.get('drink') || 'red-frontier'; }

// Load cocktail JSON and build the page
async function loadCocktail() { const drinkId = getDrinkId(); const jsonPath = cocktails/${drinkId}.json;
try { const response = await fetch(jsonPath); if (!response.ok) throw new Error('Cocktail not found.'); const data = await response.json();

// Set Meta tags
document.title = `${data.name} – Signature Cocktail`;
...
tagsDiv.innerHTML = data.tags.map(tag => `<span style="background:#ddd;padding:4px 8px;border-radius:4px;margin:3px;display:inline-block">#${tag}</span>`).join(' ');
...
if (data.variationOf) {
  ...
  variationLinks.innerHTML = `<strong>Variations:</strong> ` +
    data.variations.map(v => `<a href="cocktail.html?drink=${v}">${v.replace('-', ' ')}</a>`).join(', ');
}

} catch (err) { document.body.innerHTML = <main><h1>Error</h1><p>${err.message}</p></main>; } }
